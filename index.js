{
  const firebaseConfig = {
    ...
    databaseURL:
      "https://budgetbook-6b708-default-rtdb.europe-west1.firebasedatabase.app/",
      ...
      if (pic == undefined) {
        if (total < 0) {
          ...
          newCustomer.innerHTML =
            '<div class="ellipse"></div><div class="circle"></div><div class="cInfo"><a>' +
            cname +
            '</a><a style="color:red;font-weight:bold;">&#8377 ' +
            total +
            "</a></div>";
        } else {
          newCustomer.innerHTML =
            '<div class="ellipse"></div><div class="circle"></div><div class="cInfo"><a>' +
            cname +
            '</a><a style="color:green;font-weight:bold;">&#8377 ' +
            total +
            "</a></div>";
        }
      } else {
        if (total < 0) {
          total = total * -1;
          newCustomer.innerHTML =
            '<div class="ellipse"><img src="' +
            pic +
            '" width="100%" height="100%"></div><div class="circle1"></div><div class="cInfo"><a>' +
            cname +
            '</a><a style="color:red;font-weight:bold;">&#8377 ' +
            total +
            "</a></div>";
    ...
    if (userID == usid) {
      ...
      if (settlement < 0) {
        ...
        cinfo.innerHTML =
          ...
          '</div><div class="phone">' +
          cnumber +
          '</div><div class="youget">Due amount:<p style="color:red">&#8377; ' +
          settlement +
          "</p></div>";
      } else {
        cinfo.innerHTML =
          ...
          '</div><div class="phone">' +
          cnumber +
          '</div><div class="youget">Due amount:<p style="color:green; m">&#8377; ' +
          settlement +
          "</p></div>";
      }
    }
          ...
          window.location.href =
            ...
            encodeURIComponent(
              "Payment Reminder for " +
                ...
                ".\nPayment of rupees " +
                settlement +
                " is pending.We request you to clear the dues as soon as possible.\n-sent via LEDGITs.com"
            );
        }
      },
      errorData
        ...
        if (id1 == transactions[key[i]].cid) {
          ...
          if (money > 0) {
            ...
            newTransaction.innerHTML =
              '<div class="text">You Got<div class="inr" style="color:green;font-weight:bold;">&#8377;' +
              money +
              '</div></div><div class="DandT"><a>' +
              tdate +
              "<a><br><a>" +
              ttime +
              '</a></div><div class="hara"></div>';
            document.querySelector(".TD").appendChild(newTransaction);
          } else {
            money = money * -1;
            newTransaction.setAttribute("class", "YouGave");
            newTransaction.innerHTML =
              '<div class="text">You Gave<div class="inr" style="color:red;font-weight:bold;">&#8377;' +
              money +
              '</div></div><div class="DandT"><a>' +
              tdate +
              "</a><br><a>" +
              ttime +
              '</a></div><div class="laal"></div>';
            document.querySelector(".TD").appendChild(newTransaction);
          }
        }
      }
  ...
  $("#yougave").click(function () {
    ...
    var time = today1.toLocaleString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    var userID = firebase.auth().currentUser.uid;
  ...
  function fdisplay(money, cName, dat) {
    ...
    for (var i = 0; i < money.length; i++) {
      if (money[i] < 0) {
        ...
        custReport.innerHTML =
          '<div class="ca"><div class="gr"></div><div class="txt">' +
          nCopy[i] +
          '</div><div class="txtM">&#8377 ' +
          mon +
          '</div><div class="txtN"></div><div class="calenderff">' +
          dat[i] +
          "</div></div>";
        ...
        custReport.innerHTML =
          '<div class="ca"><div class="gr"></div><div class="txt">' +
          cName[i] +
          '</div><div class="txtM"></div><div class="txtN">&#8377 ' +
          money[i] +
          '</div><div class="calenderff">' +
          date +
          "</div></div>";
        document.querySelector(".entry").appendChild(custReport);
      }
  ...
  function message() {
    ...
    ref.on(
      ...
      function (data) {
        ...
        var message =
          "Payment Reminder from bname for " +
          cname +
          ".Payment of rupees " +
          settlement +
          " is pending.We request you to clear the dues as soon as possible.-sent via LEDGITs.com";
        if (cnumber != null) {
          if (settlement < 0) {
            settlement = settlement * -1;
          }
